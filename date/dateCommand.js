...
var processDate = (msg) => {
    ...
    let filtered = words.filter(word => !FLAGS.includes(word.toUpperCase()))
    .filter(word => word.toLowerCase() != "!date").join(" ");
    let unix = utcParse(filtered);
    if(unix == -1){
        msg.reply(mls`That was not a valid time or day, and no, I don't want to date you.`);
        return;
    }
    let out = `<t:${unix}:${flag}>`;
    msg.reply(mls`${out}
...
function utcParse(inputString){
    let step1 = chrono.parseDate(inputString);
    if(step1 == null)return -1;
    let step2 = Math.floor(step1.getTime()/1000);
    return step2;
}
